...
public class PersonDaoImpl implements PersonDao {
    ...
    public List<Person> findPersonsByFirstnameQueryDSL(final String firstname) {
        ...
        List<Person> persons = query.from(person).where(person.firstname.eq(firstname)).fetch();
        return persons;
    }
    ...
    public List<Person> findPersonsByFirstnameAndSurnameQueryDSL(final String firstname, final String surname) {
        final JPAQuery<Person> query = new JPAQuery<>(em);
        final QPerson person = QPerson.person;
        List<Person> persons = query.from(person).where(person.firstname.eq(firstname).and(person.surname.eq(surname))).fetch();
        return persons;
    }
    ...
    public List<Person> findPersonsByFirstnameInDescendingOrderQueryDSL(final String firstname) {
        final JPAQuery<Person> query = new JPAQuery<>(em);
        final QPerson person = QPerson.person;
        List<Person> persons = query.from(person).where(person.firstname.eq(firstname)).orderBy(person.surname.desc()).fetch();
        return persons;
    }
    ...
    public int findMaxAge() {
        final JPAQuery<Person> query = new JPAQuery<>(em);
        final QPerson person = QPerson.person;
        int maxAge = query.from(person).select(person.age.max()).fetchFirst();
        return maxAge;
    }
